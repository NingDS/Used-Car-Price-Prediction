...
with container1:
    ...
    brand = col1.selectbox('Car Brand', brand_list)
    
    OMV= col2.number_input('Open Market Value (SGD)')

    ARF= col3.number_input('Additional Registration Fee (SGD)')
...
with container3:
    ...
    COE_left_year= col1.number_input('Total COE Left (Year)')
    
    COE_left_mth= col2.number_input('Total COE Left (Month)')
    
    COE_left_day= col3.number_input('Total COE Left (Days)')
...
with container4:
    ...
    power= col3.number_input('Power (kW)')
...
COE_left= COE_left_year*365 + COE_left_mth*30 + COE_left_day
